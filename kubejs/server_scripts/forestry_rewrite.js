onEvent('block.right_click', event => {
   ...
   for(let i = -pitlength; i <= pitlength; i++){
      ...
      if(!level.getBlock(x+i, y-1, z+pitlength+1).hasTag('chisel:bricks')){
         player.tell(`You are missing a Brick Block at ${x+i}, ${y}, ${z+pitlength+1}`)
         valid = false
      }
      //south
      if(!level.getBlock(x+i, y-1, z-pitlength-1).hasTag('chisel:bricks')){
      ...
      for (let j = -pitlength; j <= pitlength; j++){
         if(!level.getBlock(x+i, y, z+j).hasTag('chisel:bricks')){
            if(i == 0 && j == 0){
               ...
               player.tell(`You are missing a Brick Block at ${x+i}, ${y}, ${z+j}`)
               valid = false
            }
         }
         if(!level.getBlock(x+i, y-1, z+j).hasTag('minecraft:logs')){
            player.tell(`You are missing a Log Block at ${x+i}, ${y-1}, ${z+j}`)
            valid = false
         }
      }
   }
   ...
   if(valid){
      ...
      player.tell('Charcoal Pit Ignited')
      ...
      server.scheduleInTicks(delay, entity.block, (cb) => {
         ...
         player.tell('Charcoal Pit Converted')
      });
